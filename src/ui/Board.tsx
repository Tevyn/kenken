...
export interface BoardProps {
  ...
  /**
   * The cells the hint panel's correctness check rejected. Stored state rather
   * than derived, unlike `errors`: it is what the board looked like when the
   * player asked, and it has to survive edits that would recompute a derived
   * answer.
   */
  verdict?: Verdict;
...
export function Board({
  ...
  const incorrect = useMemo(() => new Set(verdict), [verdict]);
  ...
  const cellHints = useMemo<CellHint[] | null>(() => {
    ...
    return Array.from({ length: cellCount }, (_, index) => {
      ...
      if (inCage) {
        ...
        if (row === size - 1 || cageIds[index + size] !== cage)
          classes.push('kk-cell--hint-cage-b');
    ...
    <div className="kk-board-wrap">
      ...
      */}
      <div className="kk-board__errors" aria-live="polite" aria-atomic="true">
        {errorCount > 0 &&
          `${errorCount} ${errorCount === 1 ? 'cell conflicts' : 'cells conflict'}`}
      </div>
    </div>
  );
}

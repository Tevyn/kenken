...
describe('computeCellEdges', () => {
  ...
  for (const [label, puzzle] of puzzles) {
    it(`marks a heavy edge exactly where two cages meet (${label})`, () => {
      ...
      for (let index = 0; index < size * size; index++) {
        ...
        const expectedRight =
          col < size - 1 && ids[index] !== ids[indexOf(row, col + 1, size)]
        const expectedBottom =
          row < size - 1 && ids[index] !== ids[indexOf(row + 1, col, size)]
  ...
  it('does not mark an edge between two cells of the same cage', () => {
    ...
    const left = cage!.cells.find(
      (cell) => cage!.cells.includes(cell + 1) && colOf(cell, 4) < 3,
    )!
